...
module.exports = {
  ...
  async createPost({ body, params }, res) {
    try {
      const todo = await PostItem.create({ item: body.item, userId: params.userid })
      return res.status(200).json({ status: "success", payload: todo})
    } catch(err){
      return res.status(400).json({ status: "error", msg: `Error creating Post Item: ${err.message}` })
    }
  },
  ...
  async getAllPost({ body, params }, res) {
    try {
      ...
      return res.status(400).json({ status: "error", msg: `Error retrieving Post Items: ${err.message}` })
    }
  },
  ...
  async getOnePost({ params }, res) {
    try {
      ...
      return res.status(400).json({ status: "error", msg: `Error retrieving Post Item: ${err.message}` })
    }
  },

  async updatePost({ body, params }, res) {
    try {
      const post = await PostItem.findOneAndUpdate({ _id: params.id }, { item: body.item }, { new: true })
      return res.status(200).json({ status: "success", payload: post })
    } catch(err){
      console.log(err.message)
      return res.status(400).json({ status: "error", msg: `Error updating Post Item: ${err.message}` })
    }
  },
  ...
  async deleteOnePost({ params }, res) {
    try {
      ...
      return res.status(400).json({ status: "error", msg: `Error removing Post Item: ${err.message}` })
    }
  },
